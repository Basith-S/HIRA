...
app.get("/api/cascade/status", (_req, res) => {
  res.json({
    engine: "CascadeFlow (Gemini + RawInput)",
    phase: 6,
    status: "active",
    tokenBudget: TOKEN_BUDGET,
    description:
      "CascadeFlow Routing Engine — Intake classifier (Gemini Flash) → conditional Deep Analysis (Gemini Pro), appending structured audit trail.",
    paths: {
      fast_path: "gemini-1.5-flash — classification & baseline mitigation",
      escalation_path: "gemini-1.5-pro — deep forensic analysis",
...
async function bootstrap(): Promise<void> {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`[SENTRI] Phase 6 — Gemini + RawInput Pipeline active`);
